...
## 2026-06-29 Private Upload Storage Local Migration Packet

- Added `supabase/migrations/202606290001_create_bathroom_lead_evidence_files.sql`.
- Added the private `bathroom_lead_evidence_files` table to the migration verifier private-table list.
- The migration defines private file metadata, RLS, service-role-only grants and a private storage bucket row.
- No object access policies, upload routes or admin download routes were added.
- Supabase CLI was not installed in the shell, so the migration filename follows the repo timestamp convention rather
  than being generated by `supabase migration new`.
- No SQL was applied locally, to staging or to production.

## 2026-07-01 Private Upload Storage Staging Apply Gate

- Attempted to proceed to approved local/staging Supabase apply and verification.
- Blocked because the shell only exposed `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY`.
- Missing approved target flags, anon key, database connection URL, Supabase CLI and `psql`.
- Added `docs/qa/private-upload-storage-staging-apply-gate-2026-07-01.md` to record the blocked gate.
- Updated `npm run qa:supabase:staging` coverage for `bathroom_lead_evidence_files` anon read/insert blocks and
