...
| Release-Exposure Criteria | IN_PROGRESS | Docs-only criteria for a controlled noindex Design Studio pilot; no feature-flag, sitemap, nav, indexing, measurement, AR, storage, pricing or procurement changes | Codex | 2026-06-24 |  | User-entered measurements and AR/browser-camera experiments remain locked | `git diff --check` required before PR | Instrumentation-ready only; no fabricated market results | Vincent approved starting criteria work on 2026-06-24 | Yes |
