...
| Controlled Noindex Pilot Prep | IN_PROGRESS | Docs/config-only pilot runbook and env warning; no route, nav, sitemap, indexing, measurement, AR, storage, pricing or procurement behavior changes | Codex | 2026-06-24 |  | Current single feature flag also controls nav and sitemap; pilot exposure needs a later discovery-split implementation branch | `git diff --check` required before PR | Instrumentation-ready only; no fabricated market results | Vincent approved prep work on 2026-06-24 | Yes |
