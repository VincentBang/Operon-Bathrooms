...
Phase 7 Shared Operon System Infrastructure is approved for acceptance-criteria-only planning, a docs-only shared architecture map, a docs-only shared glossary, a docs-only lifecycle vocabulary, a docs-only adapter-readiness checklist, a docs-only contract field inventory and a docs-only adapter decision packet. No Phase 7 implementation, shared package extraction, cross-repo edits, Supabase changes, Netlify changes or public exposure changes are approved.
...
1. Review the Phase 7 docs-only adapter decision packet PR.
...
Phase 7 may define shared contract candidates, architecture mapping, glossary vocabulary, lifecycle vocabulary, adapter-readiness requirements, contract field classifications and adapter source recommendations as documentation only. No shared package, cross-repo dependency or service integration is approved.
...
Review and approve the Phase 7 adapter decision packet PR, then decide whether to pause or draft docs-only destination contract acceptance criteria. Keep `/design-studio` noindex and keep public discovery locked.
