...
Phase 7 Shared Operon System Infrastructure is approved for acceptance-criteria-only planning, a docs-only shared architecture map, a docs-only shared glossary and a docs-only lifecycle vocabulary. No Phase 7 implementation, shared package extraction, cross-repo edits, Supabase changes, Netlify changes or public exposure changes are approved.
...
1. Review the Phase 7 docs-only lifecycle vocabulary PR.
...
Phase 7 may define shared contract candidates, architecture mapping, glossary vocabulary and lifecycle vocabulary as documentation only. No shared package, cross-repo dependency or service integration is approved.
...
Review and approve the Phase 7 lifecycle vocabulary PR, then decide whether to pause or draft a docs-only adapter-readiness checklist. Keep `/design-studio` noindex and keep public discovery locked.
